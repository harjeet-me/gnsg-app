...
export class EmployeeService {
  ...
  protected convertDateFromClient(employee: IEmployee): IEmployee {
    const copy: IEmployee = Object.assign({}, employee, {
      sevaStartDate: employee.sevaStartDate && employee.sevaStartDate.isValid() ? employee.sevaStartDate.toJSON() : undefined,
      sevaEndDate: employee.sevaEndDate && employee.sevaEndDate.isValid() ? employee.sevaEndDate.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((employee: IEmployee) => {
        employee.sevaStartDate = employee.sevaStartDate ? moment(employee.sevaStartDate) : undefined;
        employee.sevaEndDate = employee.sevaEndDate ? moment(employee.sevaEndDate) : undefined;
      });
    }
    return res;
  }

...
export class ProgramComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.programService
        ...
        .subscribe(
          (res: HttpResponse<IProgram[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInPrograms(): void {
    this.eventSubscriber = this.eventManager.subscribe('programListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IProgram[] | null, headers: HttpHeaders, page: number): void {
